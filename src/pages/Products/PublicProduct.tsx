...
const PublicProduct =  () => {
    ...
    const categoryAction = (Idcategory: number, category: string) => {
        ...
        const resultProducts: any = products?.filter(product => product.category_id === Idcategory )  
    ...
    return (
        <div className='container mx-auto'>
               <h1 className='p-2 mt-6 text-3xl font-bold text-center sm:mt-20 sm:text-4xl'>{config?.name}</h1>
               <div className='justify-between flex-1 mx-4 mt-10 sm:mx-0 sm:flex'>
                ...
                <div className='flex justify-end mx-4 sm:hidden'>
                    <select onChange={event => categoryAction(Number(event.target.selectedOptions[0].id), event.target.value)} className="max-w-xs select select-bordered">
                        <option disabled selected>Categorias</option>
                        <option selected id={`${0}`}>All</option>
                ...
                <div className="justify-center hidden my-10 sm:flex">
                    {categories && categories.length > 6 ? (
                        <>
                            <select onChange={event => categoryAction(Number(event.target.selectedOptions[0].id), event.target.value)} className="w-full max-w-xs select select-bordered">
                                <option disabled selected>Categorias</option>
                                <option selected id={`${0}`}>All</option>
                                {categories && categories.map(category =>(
                                    <option key={category.id} id={`${category.id}`}  value={category.name} >{category.name}</option>
                        ...
                        <>
                            ...
                            {categories && (
                                <button onClick={() => categoryAction(0, 'All')} 
                                    className={`tab tab-bordered ${buttonCategoryAction === 'All' || buttonCategoryAction === '' 
                                            ? 'tab-active' 
                                            : ''}`}>
                                    All
                                </button> 
                            ...
                            {categories && categories.map(category => (
                                <div className='tabs'>
                                    <button onClick={() => categoryAction(category.id, category.name)} 
                                        className={`tab tab-bordered ${buttonCategoryAction === category.name 
                                                    ? 'tab-active' 
                                                    : ''}`}>
                                        {category.name}
                                    </button> 
                    ...
                    <label htmlFor={`my-modal${product.id}`} className="cursor-pointer">
                        <div className="mx-4 shadow-xl sm:mx-0 card bg-base-100">
                            ...
                            <div className="text-center card-body">
                                ...
                                <p className='font-semibold'> {formatReal(product.price)}</p>
                            </div>
                        </div>
                    </label>
                        ...
                        <div className="flex items-center px-4 modal">
                            <div className=" modal-box">
                                ...
                                <p className='text-2xl font-semibold'> {formatReal(product.price)} </p>
                                <div className="modal-action">
                                <a href={`#`}>
                                    <label htmlFor={`my-modal${product.id}`}  className="btn-success btn btn-sm sm:btn btn-outline sm:btn-outline">
                                        Cancelar
                                    </label>
                                </a>
                                    <button onClick={() => openWhatsapp(product, Number(config?.phone))} className="btn btn-active btn-accent btn-sm sm:btn"> 
       ...
       {/*  {productsCategory?.length === 0 && loading && (
              <h1 className='text-center'>Não há produtos cadastrados nessa categoria!</h1> 
        )} */}
        {productsCategory?.length === 0 && loading && (
